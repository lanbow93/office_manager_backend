...
Needed: eventName | company | department | hoursNeeded | username | shifts
*/
...
router.get('/department/:id', async (request, response) => {
  try {
    const schedules = await Schedule.find({ department: request.params.id.toLowerCase() });
...
router.get('/user/:id', async (request, response) => {
  try {
    const schedules = await Schedule.find({ username: request.params.id.toLowerCase() });
    if (schedules.length === 0) {
...
/*
Purpose: Finds specific schedule based on mongo _id
Needed: :id = _id
*/
router.get('/individual/:id', async (request, response) => {
  try {
    const schedule = await Schedule.findById(request.params.id);
    if (!schedule) {
      throw new Error('No schedule found for the targeted parameter');
    }
    successfulRequest(response, 'Success', 'Successful Search', schedule);
  } catch (error) {
    failedRequest(response, 'Failed Search', 'Unable To Locate User', error);
  }
});
